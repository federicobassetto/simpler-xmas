...
export async function generateNextQuestion(sessionId: string) {
  ...
  // Fetch answers for existing questions
  ...
  const nextIndex = existingQuestions.length + 1;
...
export async function generatePlan(sessionId: string) {
  ...
  const quotes = await fetchQuotes(14);
  ...
  promptParts.push(
    "",
    "Create a 14-day advent-style plan based on the user's wish and answers."
  );
  ...
  // Calculate target dates (14 days leading up to December 25)
